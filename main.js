...
const birthdayFull = require('./commands/birthdayFull');
...
const aestheticCommand = require('./commands/aesthetic');
const bioCommand = require('./commands/bio');
const instabioCommand = require('./commands/instabio');
...
const symbolCommand = require('./commands/symbol');
...
async function handleMessages(sock, messageUpdate, printLog) {
    ...
    if (buttonId === 'channel') {
        await sock.sendMessage(chatId, {
            text: '📢 Channel link has been removed.'
        }, { quoted: message });
        return;
    }
    ...
    if (buttonId === 'support') {
        await sock.sendMessage(chatId, {
            text: '🔗 Support group is currently unavailable.'
        }, { quoted: message });
        return;
    }
}
...
break;
            ...
            case userMessage.startsWith('.lyrics'):
                ...
                break;
            case userMessage.startsWith('.wikipedia'):
            case userMessage.startsWith('.wiki'):
                const wikiArgs = userMessage.split(' ').slice(1);
                await wikipediaCommand(sock, chatId, message, wikiArgs);
                break;
            ...
            case userMessage === '.love': 
             ...
             const rmText = userMessage
             .replace('.readmore', '')
             .trim();
                await readmoreCommand(
                sock,
                chatId,
                message,
                rmText
            ...
            case userMessage.startsWith('.bday') || userMessage.startsWith('.birth') || userMessage.startsWith('.birthday'):
                ...
                break; 
            case userMessage.startsWith('.qr') || userMessage.startsWith('.qrcode'):
                await qrCommand(sock, chatId, message);
                break;
            case userMessage.startsWith('.readqr'):
                await readQrCommand(sock, chatId, message);
                break;
            ...
            case userMessage.startsWith('.setgpp'):
                ...
                break;
            case userMessage.startsWith('.instagram') || (userMessage === '.ig' || userMessage.startsWith('.ig ')):
            ...
            case userMessage.startsWith('.sudo'):
                ...
                break;
            case userMessage === '.goodmorning' || userMessage === '.morning' || userMessage === '.gm':
                await goodmorningCommand(sock, chatId, message);
                break;
            case userMessage === '.goodnight' || userMessage === '.lovenight' || userMessage === '.gn':
                await goodnightCommand(sock, chatId, message);
                break;
            case userMessage === '.aesthetic':
                await aestheticCommand(sock, chatId, message);
                break;
            case userMessage === '.bio':
                await bioCommand(sock, chatId, message);
                break;
            case userMessage === '.instabio':
                await instabioCommand(sock, chatId, message);
                break;
            case userMessage === '.caption':
            case userMessage === '.cap':
                await captionCommand(sock, chatId, message);
                break;
            case userMessage === '.nickname':
            case userMessage === '.nick':
                await nicknameCommand(sock, chatId, message);
                break;
            case userMessage === '.username':
                await usernameCommand(sock, chatId, message);
                break;
            case userMessage === '.symbol':
            case userMessage === '.sym':
                await symbolCommand(sock, chatId, message);
                break;
            case userMessage === '.quiz':
                await startQuizPoll(sock, chatId);
                break;
